...
ws['A1'].font = Font(name='Times New Roman',
                     size=14, italic=False, bold=True)
ws.column_dimensions['A'].width = 12

ws['B1'] = 'Name'
ws['B1'].font = Font(name='Times New Roman',
                     size=14, italic=False, bold=True)
ws.column_dimensions['B'].width = 22

ws['C1'] = 'Current Price'
ws['C1'].font = Font(name='Times New Roman',
                     size=14, italic=False, bold=True)
ws.column_dimensions['C'].width = 20

ws['D1'] = 'Percent Change'
ws['D1'].font = Font(name='Times New Roman', size=14,
                     italic=False, bold=True)

ws.column_dimensions['D'].width = 22

ws['E1'] = "Yesterday's Price"
ws['E1'].font = Font(name='Times New Roman',
                     size=14, italic=False, bold=True)
...
for row in table_rows[1:6]:
    ...
    if len(symbol_and_name_list) == 3:
        name = symbol_and_name_list[0] + ' ' + symbol_and_name_list[1]
    else:
    ...
    grey_font = Font(color='838B8B')
    write_sheet.cell(i, 1).value = symbol
    write_sheet.cell(i, 1).font = grey_font
    write_sheet.cell(i, 2).value = name
    write_sheet.cell(i, 2).font = grey_font
    write_sheet.cell(i, 3).value = '$' + str(round_current_price)
    write_sheet.cell(i, 3).font = grey_font
    green_font = Font(color="00C957")
    write_sheet.cell(i, 4).value = percent_change
    write_sheet.cell(i, 4).font = green_font
    write_sheet.cell(i, 5).value = '$' + str(round_yesterday_price)
    write_sheet.cell(i, 5).font = grey_font
    i += 1
...
for row in table_rows[1:6]:
    ...
    locale.setlocale(locale.LC_ALL, '')  # formatting currency
    ...
    if name == 'Bitcoin':
        if abs(float(current_price_without_dollarsign) - float(yesterday_price)) > 5:
            ...
            message = name + ' has changed to a price of ' + \
                str(current_price) + \
                " from yesterday's price of " + currency_price
